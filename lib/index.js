...
function normalizeContext(ctx) {
  var that = (0, _clone2.default)(ctx);
  that.query = (0, _clone2.default)(_loaderUtils2.default.getOptions(ctx) || {});
  delete that.query.namedExport;
  return that;
}

function delegateToCssLoader(ctx, input, callback) {
  var that = normalizeContext(ctx);
  that.async = function () {
    return callback;
  };
  _cssLoader2.default.call.apply(_cssLoader2.default, [that].concat(_toConsumableArray(input)));
}

function cssLocalsLoader() {
  var that = normalizeContext(this);
  ...
  _cssLoader2.default.call.apply(_cssLoader2.default, [that].concat(input));
}
