...
function normalizeOptions(ctx) {
  var options = _loaderUtils2.default.getOptions(ctx) || {};
...
function delegateToCssLoader(ctx, input, callback) {
  var options = normalizeOptions(ctx);
  _cssLoader2.default.call.apply(_cssLoader2.default, [_extends({}, ctx, { query: options, async: function async() {
      return callback;
    } })].concat(_toConsumableArray(input)));
}

function cssLocalsLoader() {
  var options = normalizeOptions(this);

  for (var _len = arguments.length, input = Array(_len), _key = 0; _key < _len; _key++) {
    input[_key] = arguments[_key];
  }

  _cssLoader2.default.call.apply(_cssLoader2.default, [_extends({}, this, { query: _extends({}, options, { exportOnlyLocals: true }) })].concat(input));
}
...
module.exports = function () {
  ...
  cssLocalsLoader.call.apply(cssLocalsLoader, [this].concat(input));
};